...
Production requires an exact two-parent promotion with the same Git tree as the promoted test parent. Its credential-free validation job resolves the single immutable artifact from that exact successful test run by ID, re-verifies its manifest and bytes, compares the current candidate template with the promoted test template, and republishes those exact bytes behind the current production run's ID/name/digest outputs. The OIDC job performs the same independent transport checks before using those bytes. Only dependent deployment jobs receive GitHub OIDC permission.
